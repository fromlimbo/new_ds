...
def GAVARP_Process_json(input_json):
    ...
    for item in orderjson:
        ...
        if item["start_loc_longitude"] == "" or item["start_loc_latitude"]=="" or item["end_loc_longitude"]=="" or item["end_loc_latitude"]=="" or item["priority"]=="":
            continue
        if int(item["priority"])<0 or int(item["priority"])>9:#illegal input
            continue
