...
def random_packaging(data, misc):
        ...
        while shipment_list:  # 把所有运单都装走
            ...
            for gene in gene_list:
                if gene.add_ship(ship):  # Added this ship into this gene successfully
                    break
        for gene in gene_list:
            if gene.id == 'RemainShipsContainer' or len(gene.ships) < sum(
                    gene.slot_cap):  # If the trailer in this gene is not full
                shipment_list += list(
                    gene.ships.values())  # all the shipments in this trailer are put back into shipment_list
                gene.ships = {}  # this trailer is set empty
                gene.rearrange_ships()  # gene.slot, gene._ship_loc,gene.ships are reset
        full_gene = sum(len(i.ships) == sum(i.slot_cap) for i in gene_list if
                        i.id != 'RemainShipsContainer')  # The total number of full trailers
...
def initialization(ppl_size, data, misc, flag_crossover=False, print_switch=0, old_genes=None):
    ...
    num_empty_solution = 0

    while count < ppl_size:
        ind = random_packaging(data, misc)#产生一个随机装载方案
        # if the ind generated by rand_packaging() is an empty solution
    ...
    if ppl_size > len(population) > 0:
        population.append([population[0]] * (ppl_size - len(population)))
    return population
