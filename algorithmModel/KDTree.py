...
"""
Notes:
1. leafnode: indexes whose data belong to the leafnode, which makes the tree more complicated.

Revised by Polly 2017.11.01:
1. add an interface IN "__query" to search the real distance when they are available.
2. add to_Cartesian to transfer the lat & lng to x,y,z
...
__all__ = ['to_Cartesian', 'minkowski_distance_p', 'minkowski_distance',
           'distance_matrix',
           'Rectangle', 'KDTree']
...
def to_Cartesian(lat, lng):
    ...
    :param lat:
    :param lng:
    :return: transfer the location [lat, lng] to [x, y, z]
    so that the KDTree with E-distance can be just applied.
    """
    R = 6371.004
    lat_, lng_ = map(radians, [float(lat), float(lng)])  # degree to radius
...
class KDTree(object):
    ...
    def __query(self, x, real_dist=False, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        #  __query(self, x, city_id, real_dist=False, k=1, eps=0, p=2, distance_upper_bound=np.inf)
        ...
        while q:
            ...
            if isinstance(node, KDTree.leafnode):
                # brute-force
                if real_dist & os.path.exists('c:/Anji/Project/Pinche/Retailer_RealDist.pickle'):
                    # search the real dist on the LPdist_excel with city_ID
                    # 1. load the LPdist_sheet(DataFrame)
                    # 2. search the real dist
                    # 3. if the real distance is not available, the GPS distance should be used as a replacement
                    # ds = LPdist[self.city_id[node.idx], city_id_point] # x[:, -1] = city_id
                    # city_id_point = city_id     # as an input in this function
                    # still the same//2017.09.12

                    data = self.data[node.idx]
                    ds = minkowski_distance_p(data, x[np.newaxis, :], p)
                    # if the real distance is not complete, the minkowski_distance is still useful.
    ...
    def query(self, point, real_dist=False, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        """
        ...
        real_dist: whether we have the real distance between cities
        k : int, optional
        ...
        if retshape != ():
            ...
            for c in np.ndindex(retshape):
                hits = self.__query(x[c], real_dist=False, k=k, eps=eps, p=p, distance_upper_bound=distance_upper_bound)
                #  self.__query(x[c],city_id[c],real_dist=False, k=k, eps=eps, p=p...)
                if k is None:
            ...
            hits = self.__query(x, real_dist=False, k=k, eps=eps, p=p, distance_upper_bound=distance_upper_bound)
            # self.__query(x, city_id, real_dist=False, k=k, eps=eps, p=p, distance_upper_bound=distance_upper_bound)
            if k is None:
